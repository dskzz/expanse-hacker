...
#### RFC‑2310 Quantum Key Distribution Subprofile (queued 2026-09-09, undrafted)

**Purpose**: An optional L0 media subprofile using entangled-photon distribution over a dedicated line-of-sight/fiber channel to establish a shared symmetric secret, with physical eavesdropping-detection (real QKD physics, not asymmetric crypto — a common misconception this RFC should head off explicitly in its own preface). **Why**: A physically-grounded, genuinely high-assurance session-key origin for links that can afford the dedicated channel, in the same "impressive tech, real physical cost" register as Tightbeam. **Defines**: QKD channel lifecycle (reservation/acquisition/maintenance/occlusion, mirroring RFC-2306's beam lifecycle), key material handoff into RFC-2301 §4's Session Key tier only (MUST NOT touch AK/DK/SK or any Authority Plane identity record), and the explicit doctrine that eavesdropping-detection is a tripwire on the channel, not a guarantee about the resulting key or a defense against denial-of-service (an attacker who can't read the key can still force retries/fallback). **Interaction**: consumed by RFC-2301 session negotiation; requires independent authentication from the Authority Plane (AK/NetworkCert, RFC-2362 TrustDomain) since QKD alone doesn't establish identity — a QKD key with no prior authenticated channel is exactly as MITM-vulnerable as any other symmetric key, and this RFC should say so plainly rather than implying otherwise. **Profiles**: distance/repeater-tiered service classes (a lineage without repeater infrastructure simply can't reach as far), likely reusing RFC-2306's MIAP/Dr. Selene Vargo voice given the shared alignment/occlusion physics culture. **Tests**: channel-disturbance detection, key-material isolation from Authority Plane records, fallback-under-jamming behavior.
